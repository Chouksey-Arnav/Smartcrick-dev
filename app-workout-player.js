// ================================================================
// app-workout-player.js — SmartCrick Immersive Workout Player v1.0
// Fullscreen step-by-step player with rest timers, YT ambient, audio
// ================================================================
(function() {
'use strict';
...
function RingTimer(props) {
  ...
  return h('div', { style: { position:'relative', width:200, height:200, flexShrink:0 } },
    ...
    h('div', {
      ...
      h('div', { style:{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.5)', letterSpacing:'0.12em', textTransform:'uppercase', marginTop:2 } }, 'secs')
    )
  );
}

// ─── YouTube Ambient Iframe ───────────────────────────────────
function YTAmbient(props) {
  var videoId = props.videoId;
  var iframeRef = useRef(null);
  var pausedRef = props.pausedRef;

  // Expose pause/resume control
  useEffect(function() {
    if (!pausedRef) return;
    var iframe = iframeRef.current;
    if (!iframe) return;
    try {
      var cmd = pausedRef.current
  ...
  var src = 'https://www.youtube.com/embed/' + videoId +
    ...
    '&playsinline=1&enablejsapi=1&rel=0&showinfo=0&modestbranding=1';
  return h('iframe', {
    ref: iframeRef,
    src: src,
...
function WorkoutPlayerPage(props) {
  ...
  // Derive exercises from the database
  var exercises = useRef(
    workout && A.getWorkoutExercises ? A.getWorkoutExercises(workout) : []
  ).current;

  // State
  var [status, setStatus]   = useState('preview');    // preview|set_active|set_rest|ex_rest|done
  ...
  var [ytPaused, setYtPaused] = useState(false);
  var ytPausedRef = useRef(false);

  var elapsedRef  = useRef(0);
  var statusRef   = useRef('preview');
  ...
  var videoId   = videoMap[workout.target] || videoMap['full-body'] || '';
  ...
  if (status === 'preview') {
    return h('div', {
      ...
      h(YTAmbient, { videoId:videoId, pausedRef:ytPausedRef }),
      h(PauseBtn,  { paused:ytPaused, onToggle:function() { setYtPaused(function(p) { return !p; }); } }),

      // Dark gradient overlay
      h('div', { style:{
        position:'fixed', inset:0, zIndex:1,
        background:'radial-gradient(ellipse at center, rgba(6,11,20,0.55) 0%, rgba(6,11,20,0.9) 100%)',
      }}),
      ...
      h('div', { style:{
        ...
        h('div', { style:{ marginBottom:20 } },
          h('div', { style:{ fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.5)', letterSpacing:'0.14em', textTransform:'uppercase', marginBottom:6 } }, workout.level + ' · ' + workout.target.replace('-', ' ')),
        ...
        h('div', { style:{
          ...
          borderRadius:16, border:'1px solid rgba(255,255,255,0.08)', padding:'12px 0',
        }},
          exercises.map(function(e, i) {
            return h('div', {
              ...
              h('div', {
                style:{ width:28, height:28, borderRadius:'50%', background:'rgba(255,255,255,0.07)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:800, color:'rgba(255,255,255,0.5)', flexShrink:0 }
              }, i+1),
        ...
        h('button', {
          ...
          style:{
            ...
            background:'linear-gradient(135deg,' + lvlGrad + ')',
            color:'#fff', boxShadow:'0 6px 28px rgba(0,0,0,0.5)',
  ...
  if (status === 'done') {
    ...
    return h('div', {
      style:{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
              minHeight:'100dvh', background:'#060b14', padding:'32px 24px', textAlign:'center' }
    },
  ...
  // ── ACTIVE / REST SCREENS — need ex ─────────────────────────
  if (!ex) {
    // Safety — if exercise list is empty, just go to done
    if (status !== 'done') {
      setStatus('done');
    }
    return null;
  ...
  if (isResting) {
    ...
    return h('div', {
      ...
      h('div', { style:{
        position:'relative', zIndex:2, display:'flex', flexDirection:'column',
        alignItems:'center', justifyContent:'center', minHeight:'100dvh', padding:'32px 24px',
        textAlign:'center',
      }},
        ...
        h('div', { style:{ marginTop:28, marginBottom:12 } },
          ...
          h('div', { style:{ fontSize:18, fontWeight:800, color:'#f0fdf4' } }, nextEx ? nextEx.name : 'Last exercise!'),
        ...
        h('button', {
          ...
          style:{
            marginTop:24, padding:'13px 32px', background:'rgba(255,255,255,0.08)',
            border:'1px solid rgba(255,255,255,0.2)', borderRadius:12,
            color:'#e2e8f0', cursor:'pointer', fontFamily:'inherit', fontSize:14, fontWeight:700,
          }
        }, 'Skip Rest →')
      )
    );
  ...
  var exCue = ex.duration_secs
    ? ex.duration_secs + ' seconds'
    : ex.reps + ' reps';
  ...
  return h('div', {
    ...
    h('div', { style:{
      ...
      h('div', { style:{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', paddingBottom:20 } },
        ...
        h('h2', { style:{ fontSize:34, fontWeight:900, color:'#f8fafc', lineHeight:1.1, margin:'0 0 8px', letterSpacing:'-0.02em' } },
          ex.name
        ),
        h('div', { style:{
          ...
          h('span', { style:{ fontSize:20, fontWeight:900, color:'#4ade80' } }, exCue),
        ),
...
console.log('[SC] app-workout-player v1.0 — immersive player ready');
})();
